...
1. Risks
    ...
    c. Chronic Illness
    d. Contact With Confirmed COVID 19 Patient (and Date of Contact)
    e. Contact with person Under Investigation of having COVID-19 (and Date of Contact)
2. Symptoms
    ...
    b. Secondary (2+ of these indicates the client may have COVID-19)
        ...
        * Sore throat
3. Diagnosis
    a. Tested (plus Results and Test Date)
    b. Under Investigation (plus Date Notified)
...
Out of **`r nrow(basic)`** clients screened for COVID-19, **`r CDCsymptomatic$Clients[CDCsymptomatic$MayHaveCOVID19CDC == TRUE]`** have symptoms that indicate this client "may have COVID-19" at the time of their assessment. This is defined in the [CDC's guidelines](https://www.cdc.gov/coronavirus/2019-ncov/symptoms-testing/symptoms.html) as having at least one Primary Symptom or at least 2 Secondary Symptoms.
...
plot_a %>%
  select(PersonalID, COVID19AssessmentDate, MayHaveCOVID19CDC) %>%
  group_by(COVID19AssessmentDate, MayHaveCOVID19CDC) %>%
  summarise(Clients = n()) %>%
  ggplot(aes(x = COVID19AssessmentDate, y = Clients,
             fill = MayHaveCOVID19CDC)) +
  geom_bar(stat = "identity", 
           show.legend = FALSE,
           position = position_stack(reverse = TRUE)) +  
  scale_fill_brewer(palette = "Dark2") +
  theme_classic() +
  ggtitle("Clients who May Have COVID-19 According to CDC Guidelines") +
