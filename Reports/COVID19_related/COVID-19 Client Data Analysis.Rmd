...
priority <- covid19 %>%
  mutate(
    ...
    Priority = factor(Priority, levels = c("High", "Low"))
  )

```

Our Prioritization report for Coordinated Entry will use this metric to determine whether a client should be prioritized for immediate non-congregate housing based on their risks, symptoms, contact with others, and test results. This report is used to help communities with moving households into permanent housing. High Priority would mean the household should be moved out of a shelter immediately for quarantine due to a suspected or unconfirmed infection, isolation because the person has tested positive, or non-congregate sheltering as the person is at high risk of poor outcomes if infected with COVID-19.
...
priority_plot %>%
  ...
  scale_fill_manual(values = c("#43a2ca", "#bae4bc")) +
  geom_bar(stat = "identity") +
  theme_classic() +
