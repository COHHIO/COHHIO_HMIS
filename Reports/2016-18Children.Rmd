...
## Individual Children {.tabset .tabset-fade}
...
plot_ly(
  ...
  add_trace(y = ~ school_age, name = "Ages 6 to 17") %>%
  layout(
    title = "Children in Emergency Shelters or Unsheltered Outreach",
    xaxis = list(title = "Year"),
    yaxis = list(title = "Children Served"),
    barmode = "stack"
  )
...
## Top 35 Counties Serving Children
...
countylist <- full_join(childs, households, by = c("Region", "CountyServed")) %>%
  ...
  select(Region, County, Children) 

gt(head(countylist, n = 35L))
