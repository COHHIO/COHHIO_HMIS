...
small_project <- Project %>%
  filter(ProjectType %in% c(project_types_w_beds, 13) &
...
BedCapacity <- small_project %>%
  ...
  summarise(BedInventory = sum(BedInventory),
            BedEndDate = max(ymd(BedEndDate)),
            BedStartDate = min(ymd(BedStartDate))) %>%
  ungroup() %>%
...
Utilizers <- small_enrollment %>%
  ...
  filter(int_overlaps(StayWindow, FilePeriod))
...
Utilization <- BedCapacity %>%
  ...
  mutate(
    ...
    UtilizationFAM = percent(BedNightsServedFAM / PossibleFAM, accuracy = .1),
    UtilizationIND = percent(BedNightsServedIND / PossibleIND, accuracy = .1),
    UtilizationALL = percent((BedNightsServedFAM + BedNightsServedIND) /
  ...
  arrange(desc((BedNightsServedFAM + BedNightsServedIND) /
                 (PossibleFAM + PossibleIND))) %>%
  select(
    ProjectID,
    ProjectName,
